...
export const fetchMatrix = async (projectId: string): Promise<TraceabilityMatrix> => {
  try {
    const response = await safeApiCall(
      apiClient.GET('/api/v1/projects/{id}/matrix', { params: { path: { id: projectId } } })
    )
    return response.data as TraceabilityMatrix || {
      items: [],
      links: [],
...
export const exportMatrix = async (projectId: string, format: 'csv' | 'json' | 'xlsx'): Promise<Blob> => {
  try {
    const response = await safeApiCall(
