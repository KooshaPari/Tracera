...
function ItemsTableView({ projectId, view, type }: ItemsTableViewProps = {}): JSX.Element {
  ...
  const [searchQuery, setSearchQuery] = React.useState(itemsTableConstants.EMPTY_STRING);
  ...
  const closeCreateModal = React.useCallback((): void => {
    ...
    navigate({
      search: (prev) => mergeSearch(prev, { [itemsTableConstants.SEARCH_PARAM_ACTION]: undefined }),
    });
  ...
  const handleProjectFilterChange = React.useCallback(
    (value: string): void => {
      ...
      navigate({
        search: (prev) =>
          mergeSearch(prev, { [itemsTableConstants.SEARCH_PARAM_PROJECT]: nextValue }),
      });
    },
    [navigate],
  );
  ...
  const handleTypeFilterChange = React.useCallback(
    (value: string): void => {
      const nextValue = itemsTableFormatters.getFilterValue(value);
      navigate({
        search: (prev) => mergeSearch(prev, { [itemsTableConstants.SEARCH_PARAM_TYPE]: nextValue }),
      });
    },
    [navigate],
  );
