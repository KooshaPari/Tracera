...
const updateConversationMessage = (
	...
	for (const conversation of conversations) {
		...
		const nextMessages = updateMessagesById(
...
const createUIActions = (set: StoreSetter): Pick<
	ChatState,
		| "setBubblePosition"
		| "setMode"
		| "setOpen"
...
const createConversationSelectionActions = (
	...
	ChatState,
	"getActiveConversation" | "setActiveConversation" | "setConversationSessionId" | "updateConversationTitle"
> => ({
...
const createMessageActions = (set: StoreSetter): Pick<
	ChatState,
		| "addMessage"
		| "setMessageStreaming"
		| "updateMessage"
...
const createStreamingActions = (set: StoreSetter, get: StoreGetter): Pick<
	ChatState,
	"setAbortController" | "setStreaming" | "stopStreaming"
> => ({
...
const createModelActions = (set: StoreSetter): Pick<
	ChatState,
	"setSelectedModel" | "setSystemPromptOverride"
> => ({
	setSelectedModel: (model) => set({ selectedModel: model }),
	setSystemPromptOverride: (value) => set({ systemPromptOverride: value }),
});

const createContextActions = (set: StoreSetter): Pick<
	ChatState,
	"setContext"
> => ({
...
export const useChatStore = create<ChatState>()(
	persist(
		(set, get) => buildChatStore(set, get),
		{
