...
describe('useItems hooks', () => {
  ...
  describe('useItems', () => {
    ...
    it('should fetch items with project filter', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => mockItems.filter(item => item.project_id === 'proj-1'),
      } as Response)
    ...
    it('should cache results with same filters', async () => {
      ...
      const { result: result1 } = renderHook(() => useItems({ projectId: 'proj-1' }), { wrapper })
      await waitFor(() => expect(result1.current.isSuccess).toBe(true))

      // Reset mock count after first fetch
      const callsBefore = mockFetch.mock.calls.length

      const { result: result2 } = renderHook(() => useItems({ projectId: 'proj-1' }), { wrapper })
    ...
    it('should refetch with different filters', async () => {
      ...
      const { result: result2 } = renderHook(() => useItems({ projectId: 'proj-2' }), { wrapper })
      await waitFor(() => expect(result2.current.isSuccess).toBe(true))
  ...
  describe('useItem', () => {
    ...
    it('should refetch when id changes', async () => {
      ...
      const { result, rerender } = renderHook(
        ({ id }) => useItem(id),
        { wrapper, initialProps: { id: 'item-1' } }
      )
  ...
  describe('integration scenarios', () => {
    it('should handle create then fetch workflow', async () => {
      ...
      const { result: createResult } = renderHook(() => useCreateItem(), { wrapper })
      ...
      const { result: listResult } = renderHook(() => useItems({ projectId: 'proj-1' }), { wrapper })
    ...
    it('should handle update then fetch workflow', async () => {
      ...
      const { result: updateResult } = renderHook(() => useUpdateItem(), { wrapper })
      ...
      const { result: itemResult } = renderHook(() => useItem('item-1'), { wrapper })
