...
describe("client-core legacy API helpers", () => {
  ...
  it("forwards each helper method, path, and init to the typed request boundary", async () => {
    ...
    await expect(clientCore.apiClient.get("/api/traces", getInit)).resolves.toEqual({
      id: "get-result",
    });
    await expect(clientCore.apiClient.post("/api/traces", postInit)).resolves.toEqual({
      id: "post-result",
    });
    await expect(clientCore.apiClient.put("/api/traces/42", putInit)).resolves.toEqual({
      id: "put-result",
    });
