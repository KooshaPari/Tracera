...
import type { Item, CreateItemInput, UpdateItemInput } from '../../api/types'
...
describe('ItemsStore', () => {
  ...
  describe('optimisticCreate', () => {
    ...
    it('should set default status if not provided', () => {
      ...
      const { items } = useItemsStore.getState()
      const tempItem = items.get('temp-1')
      expect(tempItem?.status).toBe('pending')
    })
  ...
  describe('confirmUpdate', () => {
    ...
    it('should replace item with server response', () => {
      ...
      const serverItem = { ...mockItem, title: 'Server Title', updated_at: '2024-01-02T00:00:00Z' }
