...
describe('Search Page', () => {
  ...
  describe('Search Filters', () => {
    ...
    it('filters by project', async () => {
      ...
      vi.mocked(fetchSearchResults).mockResolvedValue({
        data: [
          { id: 'r1', title: 'Result 1', projectId: 'proj-1', project: 'Project A' },
          { id: 'r2', title: 'Result 2', projectId: 'proj-2', project: 'Project B' },
        ],
        total: 2,
        query: 'test',
      })
    ...
    it('filters by date range', async () => {
      ...
      const dateFilter = screen.getByRole('button', { name: /filter by date/i })
    ...
    it('combines multiple filters', async () => {
      ...
      vi.mocked(fetchSearchResults).mockResolvedValue({
        data: [
          { id: 'r1', type: 'feature', projectId: 'proj-1', updated_at: '2024-06-01' },
          { id: 'r2', type: 'bug', projectId: 'proj-1', updated_at: '2024-06-01' },
          { id: 'r3', type: 'feature', projectId: 'proj-2', updated_at: '2024-06-01' },
        ],
        total: 3,
        query: 'test',
      })
  ...
  describe('Search Suggestions', () => {
    it('shows suggestions while typing', async () => {
      const { fetchSearchResults, searchSuggestions } = await import('@/api/search')
      const { fetchProjects } = await import('@/api/projects')
  ...
  describe('Faceted Search', () => {
    ...
    it('filters by clicking facet', async () => {
      ...
      const facetButton = screen.getByRole('button', { name: /feature \(45\)/i })
  ...
  describe('Advanced Search', () => {
    ...
    it('builds complex query with AND/OR operators', async () => {
      ...
      await waitFor(() => {
        expect(history.location.search).toContain('q=%28feature+OR+bug%29+AND+status%3Aactive')
      })
    })
  })
  ...
  describe('Loading States', () => {
    it('shows loading skeleton while searching', async () => {
      ...
      vi.mocked(fetchSearchResults).mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve({ data: [], total: 0, query: 'test' }), 100)
          )
      )
      vi.mocked(fetchProjects).mockResolvedValue({ data: [], total: 0 })
  ...
  describe('Error Handling', () => {
    it('displays error message on search failure', async () => {
      ...
      vi.mocked(fetchSearchResults).mockRejectedValue(new Error('Search service unavailable'))
      vi.mocked(fetchProjects).mockResolvedValue({ data: [], total: 0 })
  ...
  describe('Accessibility', () => {
    ...
    it('announces search results count', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('status', { name: /search results/i })).toHaveTextContent('1 result')
      })
    })
  })
})
