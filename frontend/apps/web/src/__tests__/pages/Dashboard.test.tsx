...
describe('Dashboard Page', () => {
  ...
  describe('Page Rendering', () => {
    ...
    it('renders recent activity timeline', async () => {
      ...
      vi.mocked(fetchRecentItems).mockResolvedValue({
        data: [
          { id: 'i1', title: 'Bug Fix', type: 'bug', updated_at: '2024-01-10T10:00:00Z' },
          { id: 'i2', title: 'Feature Request', type: 'feature', updated_at: '2024-01-10T09:00:00Z' },
          { id: 'i3', title: 'Documentation', type: 'doc', updated_at: '2024-01-10T08:00:00Z' },
        ],
        total: 3,
      })
      vi.mocked(fetchSystemStatus).mockResolvedValue({ status: 'healthy' })
  ...
  describe('Loading States', () => {
    ...
    it('shows loading for individual sections independently', async () => {
      ...
      await waitFor(() => {
        ...
        expect(within(projectSection).queryByRole('status')).not.toBeInTheDocument()
      })
  ...
  describe('Error Handling', () => {
    it('displays error message when projects fail to load', async () => {
      ...
      vi.mocked(fetchProjects).mockRejectedValue(new Error('Failed to fetch projects'))
      vi.mocked(fetchRecentItems).mockResolvedValue({ data: [], total: 0 })
      vi.mocked(fetchSystemStatus).mockResolvedValue({ status: 'healthy' })
    ...
    it('displays error when system status fails', async () => {
      ...
      vi.mocked(fetchProjects).mockResolvedValue({ data: [], total: 0 })
      vi.mocked(fetchRecentItems).mockResolvedValue({ data: [], total: 0 })
      vi.mocked(fetchSystemStatus).mockRejectedValue(new Error('System unavailable'))
    ...
    it('allows retry on failed sections', async () => {
      ...
      vi.mocked(fetchProjects).mockRejectedValueOnce(new Error('Network error'))
        .mockResolvedValueOnce({ data: [{ id: '1', name: 'Project Recovered' }], total: 1 })
      vi.mocked(fetchRecentItems).mockResolvedValue({ data: [], total: 0 })
      vi.mocked(fetchSystemStatus).mockResolvedValue({ status: 'healthy' })
  ...
  describe('User Navigation', () => {
    ...
    it('navigates to item detail from recent activity', async () => {
      ...
      vi.mocked(fetchProjects).mockResolvedValue({ data: [], total: 0 })
      vi.mocked(fetchRecentItems).mockResolvedValue({
        data: [{ id: 'item-456', title: 'Recent Item', type: 'feature', projectId: 'proj-123' }],
        total: 1,
      })
      vi.mocked(fetchSystemStatus).mockResolvedValue({ status: 'healthy' })
  ...
  describe('Accessibility', () => {
    it('has proper ARIA labels on interactive elements', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: /create project/i })).toHaveAttribute('aria-label')
      })
  ...
  describe('Filters and Sorting', () => {
    ...
    it('sorts projects by date', async () => {
      ...
      await userEvent.click(screen.getByRole('option', { name: /oldest first/i }))
