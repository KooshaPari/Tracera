...
const buildHierarchicalPosition = (
	index: number,
	nodeCount: number,
): { x: number; y: number } => {
...
const buildNodeData = (
	...
	const created = new Date(
		DEFAULT_YEAR,
		0,
		Math.floor(rng.next() * DAYS_IN_YEAR),
	).toISOString();
...
const pickHierarchicalTarget = (
	...
	if (childStart < nodeCount && rng.next() < CHILD_RELATION_BIAS) {
		return childStart +
			(childEnd > childStart && rng.next() > DEFAULT_DENSITY ? 1 : 0);
	}
...
const pickTargetIndex = (
	...
	const { distribution, density, nodeCount, nodes, seededRandom, sourceIdx } =
		options;
...
const buildEdges = (
	...
	const { density, distribution, edgeCount, nodeCount, nodes, seededRandom } =
		options;
...
export const calculateGraphMetrics = (
	...
	const totalDegree = [...degrees.values()].reduce(
