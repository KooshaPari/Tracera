 ...
 */
export function slugToDisplayName(slugOrName: string): string {
...
export function getProjectDisplayName(project: {
	...
	// Pattern matches: Proj_*, proj_* (case-insensitive), with optional trailing 's'
	if (project.name && !project.name.match(/^[Pp]roj_[a-z0-9_]+s?$/i)) {
		return project.name;
	}

	// Otherwise, convert the ID to a human-readable name
	return slugToDisplayName(project.id);
}
