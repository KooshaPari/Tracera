  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${API_BASE}/api/projects/${projectId}/contracts`
      )
...
export function useFeatures(projectId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${API_BASE}/api/projects/${projectId}/features`
      )
...
export function useSpecificationNavigation() {
  ...
  return {
    toADRs: (projectId: string) =>
      navigate({ to: '/specifications/adrs', search: { projectId } }),
    toADRDetail: (adrId: string) =>
      navigate({ to: `/adr/${adrId}` }),
    ...
    toFeatureDetail: (featureId: string) =>
      navigate({ to: `/feature/${featureId}` }),
