...
// A same-origin relative path (e.g. `/api`) means the page and API share a
// host. That's always a public HTTPS base when the page itself is served
// from a public HTTPS origin, so accept it in both dev and prod.
if (raw.startsWith("/")) {
  console.log(`PASS API base policy (same-origin ${raw})`);
  process.exit(0);
}
...
const ipv6Loopback = (value) => {
  ...
  const mappedIpv4 = value.includes(".")
  ...
  const expanded =
    marker >= 0
      ? [
          ...
          ...new Array(9 - parts.length).fill("0"),
          ...parts.slice(marker + 1),
        ]
      : parts;
...
const privateNetwork =
  loopback || privateIpv4(hostname) || hostname.endsWith(".local");

assert.ok(
  ["http:", "https:"].includes(url.protocol),
  `unsupported API protocol: ${url.protocol}`,
);
...
if (production && (url.protocol !== "https:" || privateNetwork)) {
  throw new Error(
    `production Pages requires a public HTTPS API base, got ${raw}`,
  );
}
