    ...
    <div ref={ref} className={cn('space-y-2', className)} {...props}>
      {React.Children.map(children, (child) => {
        if (React.isValidElement(child)) {
          const childProps: CollapsibleChildProps = { open };
          if (onOpenChange !== undefined) childProps.onOpenChange = onOpenChange;
          return React.cloneElement(child as React.ReactElement<CollapsibleChildProps>, childProps);
        }
        return child;
      })}
    </div>
