...
test("locales - uz", () => {
	...
	expect(invalidType.error!.issues[0].message).toBe(
		"Noto‘g‘ri kirish: kutilgan raqam, qabul qilingan string",
	);
	...
	expect(invalidType2.error!.issues[0].message).toBe(
		"Noto‘g‘ri kirish: kutilgan string, qabul qilingan raqam",
	);
	...
	expect(invalidValue.error!.issues[0].message).toBe(
		'Noto‘g‘ri variant: quyidagilardan biri kutilgan "a"|"b"',
	);
	...
	expect(tooSmall.error!.issues[0].message).toBe(
		"Juda kichik: kutilgan number >=10",
	);
	...
	expect(invalidFormatRegex.error!.issues[0].message).toContain(
	...
	expect(invalidFormatStartsWith.error!.issues[0].message).toContain(
		'"abcd" bilan boshlanishi kerak',
	);

	const notMultipleOf = z.number().multipleOf(3).safeParse(10);
	expect(notMultipleOf.error!.issues[0].code).toBe("not_multiple_of");
	expect(notMultipleOf.error!.issues[0].message).toContain(
		"3 ning karralisi bo‘lishi kerak",
	);
	...
	expect(unrecognizedKeys.error!.issues[0].message).toContain(
		'Noma’lum kalit: "c"',
	);
	...
	const invalidFormatEndsWith = z
		.string()
		.endsWith("xyz")
		.safeParse("invalid-string");
	expect(invalidFormatEndsWith.error!.issues[0].code).toBe("invalid_format");
	expect(invalidFormatEndsWith.error!.issues[0].message).toContain(
		'"xyz" bilan tugashi kerak',
	);

	const invalidFormatIncludes = z
		.string()
		.includes("test")
		.safeParse("invalid-string");
	expect(invalidFormatIncludes.error!.issues[0].code).toBe("invalid_format");
	expect(invalidFormatIncludes.error!.issues[0].message).toContain(
		'"test" ni o‘z ichiga olishi kerak',
	);
	...
	expect(invalidFormatEmail.error!.issues[0].message).toContain(
	...
	expect(unrecognizedKeysMultiple.error!.issues[0].code).toBe(
		"unrecognized_keys",
	);
	expect(unrecognizedKeysMultiple.error!.issues[0].message).toContain(
		"Noma’lum kalitlar",
	);
