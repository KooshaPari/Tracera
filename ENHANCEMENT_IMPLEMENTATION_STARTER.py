...
try:
    ...
    raise ImportError("Install with: bun add sentence-transformers")

try:
    import spacy
except ImportError:
    raise ImportError("Install with: python -m spacy download en_core_web_sm")

try:
    import networkx as nx
except ImportError:
    raise ImportError("Install with: bun add networkx")

try:
    from icontract import require, ensure, invariant
except ImportError:
    raise ImportError("Install with: bun add icontract")
...
class SemanticAnalysisService:
    ...
    async def calculate_semantic_similarity(
        self,
        req1_description: str,
        req2_description: str
    ) -> float:
        """
...
class QualityAnalysisService:
    ...
    AMBIGUOUS_WORDS = {
        ...
        'quickly', 'slowly', 'fast', 'slow', 'large', 'small',
        'good', 'bad', 'nice', 'easy', 'difficult', 'possible',
        'reasonable', 'typical', 'often', 'generally', 'usually',
        'several', 'many', 'few', 'a lot', 'a little'
    }

    ACTION_VERBS = {
        'shall', 'should', 'must', 'will', 'can', 'may',
        'implement', 'provide', 'create', 'generate', 'process'
    }
    ...
    async def analyze_requirement(
        ...
        # Detect action verbs
        has_action_verb = any(
            token.text.lower() in self.ACTION_VERBS for token in doc
        )
    ...
    def _calculate_complexity(self, doc) -> str:
        ...
        if token_count < 20 and avg_tokens_per_sent < 15:
            return "simple"
        elif token_count < 50 and avg_tokens_per_sent < 20:
            return "moderate"
        else:
            return "complex"
    ...
    def _generate_recommendations(
        ...
        if ambiguous_terms:
            recommendations.append(
                f"Replace ambiguous terms: {', '.join(ambiguous_terms)}"
            )
        ...
        if not has_action_verb:
            recommendations.append(
                "Add action verb (shall, must, should, will, can)"
            )
        ...
        if complexity_level == "complex":
            recommendations.append(
                "Simplify or split requirement into multiple items"
            )
...
class ImpactAnalysisService:
    ...
    async def build_graph(
        ...
        for link in links:
            graph.add_edge(
                link["source_id"],
                link["target_id"],
                relationship=link.get("link_type", "depends_on")
            )
    ...
    async def calculate_impact(
        ...
        return ImpactAnalysis(
            ...
            impacted_item_ids=sorted(list(impacted - {item_id})),
            impact_depth=depth,
...
class RequirementEnhancementService:
    ...
    async def enhance_requirement(
        ...
        item_id: str,
        description: str,
        capabilities: Optional[List[EnhancementCapability]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def find_duplicates(
        ...
        Returns:
            List of DuplicateCandidate objects
        """
        return await self.semantic_service.find_duplicate_requirements(
...
async def example_basic_quality_analysis():
    ...
    result = await service.enhance_requirement(
        item_id="REQ-001",
...
async def example_impact_analysis():
    ...
    print(f"Impact of changing REQ-001:")
