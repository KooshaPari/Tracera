...
class ExecutionEnvironmentConfig(Base, TimestampMixin):
    ...
    docker_image: Mapped[str] = mapped_column(
        String(255), nullable=False, default="node:20-alpine"
    )
    resource_limits: Mapped[dict[str, Any] | None] = mapped_column(
    ...
    # Playwright settings
    playwright_browser: Mapped[str] = mapped_column(
        String(50), nullable=False, default="chromium"
    )
    playwright_headless: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True
    )
    playwright_viewport_width: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1280
    )
    playwright_viewport_height: Mapped[int] = mapped_column(
        Integer, nullable=False, default=720
    )
    playwright_video_size: Mapped[dict[str, Any] | None] = mapped_column(
        JSONType, nullable=True
    )
    ...
    codex_full_auto: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    codex_timeout: Mapped[int] = mapped_column(
    ...
    max_artifact_size_mb: Mapped[int] = mapped_column(
