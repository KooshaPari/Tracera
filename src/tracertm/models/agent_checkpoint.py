...
class AgentCheckpoint(Base, TimestampMixin):
    ...
    sandbox_snapshot_s3_key: Mapped[Optional[str]] = mapped_column(String(1024), nullable=True)

    # Checkpoint metadata
    # - model used, token counts, duration, etc.
    checkpoint_metadata: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
