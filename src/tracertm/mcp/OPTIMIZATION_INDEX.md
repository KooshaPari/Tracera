...
3. **`tools/items_optimized.py`** - Optimized item tools (v2)
   - `create_item_v2()` - 88% fewer tokens
   - `get_item_v2()` - Minimal response
   - `query_items_v2()` - Optimized query
   - `update_item_v2()` - Lean update
   - `delete_item_v2()` - Minimal confirmation
   - `summarize_view_v2()` - Compact summary
...
# Minimal response
item = await get_item_v2("abc12345")
...
from tracertm.mcp.tools.items_optimized import query_items_v2  # v2
...
items = await query_items_v2(limit=20)
...
```python
result = await get_item_v2("abc123")
...
item = await get_item_v2(item_id, include_metadata=True)

# Don't need metadata? Omit it (default)
item = await get_item_v2(item_id)  # Fewer tokens
```
