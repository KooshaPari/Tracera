...
- `create_item_v2()` - Create with lean response
- `get_item_v2()` - Get with minimal fields
- `query_items_v2()` - Query with optimized response
...
result = await query_items_v2(...)  # Use v2
```
...
item = await get_item_v2(item_id)  # No metadata = fewer tokens
```
