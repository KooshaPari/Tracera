...
class GitHubProjectRepository:
    ...
    async def get_by_id(self, github_project_id: str) -> Optional[GitHubProject]:
    ...
    async def get_by_project_id(self, project_id: str) -> list[GitHubProject]:
        """Get all GitHub Projects for a TraceRTM project."""
        result = await self.db.execute(
            select(GitHubProject).where(GitHubProject.project_id == project_id)
        )
