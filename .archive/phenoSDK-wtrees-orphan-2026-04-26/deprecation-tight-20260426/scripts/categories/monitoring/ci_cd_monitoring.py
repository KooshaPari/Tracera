...
class CICDMonitor:
    ...
    def _validate_workflow_config(self, workflow_file: Path) -> None:
        ...
        try:
            ...
            missing_fields = [
                field for field in required_fields if field not in workflow_config
            ]
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if critical_metrics:
            recommendations.append(
                f"Address {len(critical_metrics)} critical alerts immediately",
            )
        ...
        cpu_metrics = [
            m for m in self.metrics if m.name == "system_cpu_usage" and m.value > 80
        ]
        if cpu_metrics:
            recommendations.append(
        ...
        memory_metrics = [
            m for m in self.metrics if m.name == "system_memory_usage" and m.value > 80
        ]
        if memory_metrics:
            recommendations.append(
        ...
        coverage_metrics = [
            m for m in self.metrics if m.name == "test_coverage" and m.value < 80
        ]
        if coverage_metrics:
            recommendations.append(
                "Low test coverage detected - increase test coverage",
            )
    ...
    def _calculate_health_score(self) -> float:
        ...
        high_cpu = len(
            [m for m in self.metrics if m.name == "system_cpu_usage" and m.value > 90],
        )
        score -= high_cpu * 10
        ...
        high_memory = len(
            [
                ...
                if m.name == "system_memory_usage" and m.value > 90
            ],
        )
