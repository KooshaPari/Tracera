...
class SecurityPolicyEnforcer:
    ...
    def _is_weak_password(self, password: str, policies: dict[str, Any]) -> bool  # noqa: PLR0911:
        ...
        if policies["require_special"] and not re.search(
            r'[!@#$%^&*(),.?":{}|<>]', password,
        ):
            return True
            ...
            try:
                with open(py_file, encoding="utf-8") as f:
                    ...
                    for i, line in enumerate(lines, 1):
                        ...
                        if re.search(r"http://", line) and not re.search(
                            r"localhost|127\.0\.0\.1", line,
                        ):
                            self.violations.append(
                                SecurityViolation(
                                    severity="high",
                        ...
                        if re.search(
                            r"requests\.(get|post|put|delete)", line,
                        ) and not re.search(r"auth|headers.*Authorization", line):
                            self.violations.append(
                                SecurityViolation(
                                    severity="medium",
                                    category="api_security",
    ...
    def _generate_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "files_affected": len({v.file for v in self.violations}),
        }
...
"""
        ...
        if report["violations"]:
            ...
            if len(report["violations"]) > 10:
                output += (
                    f"  ... and {len(report['violations']) - 10} more violations\n"
                )
