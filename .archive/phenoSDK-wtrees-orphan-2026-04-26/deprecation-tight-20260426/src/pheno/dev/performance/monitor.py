...
class PerformanceMonitor:
    ...
    def __init__(self, enable_detailed_metrics: bool = True):
        ...
        if not PSUTIL_AVAILABLE and enable_detailed_metrics:
            logger.warning(
                "psutil not available - detailed metrics disabled. "
                "Install with: pip install psutil",
            )

        logger.info(
            f"PerformanceMonitor initialized (detailed_metrics={self.enable_detailed_metrics})",
        )
    ...
    def measure_operation(self, operation_name: str, provider_name: str = "", model_name: str = ""):
        ...
        try:
            ...
            logger.exception(
                f"Operation failed: {operation_name} (correlation_id={correlation_id}): {e}",
            )
            raise
            ...
            metrics.finalize(success=metrics.success)
    ...
    async def measure_async_operation(
        ...
        try:
            logger.debug(
                f"Starting async operation: {operation_name} (correlation_id={correlation_id})",
            )
            yield metrics
            ...
            metrics.finalize(success=False, error=e)
            logger.exception(
                f"Async operation failed: {operation_name} (correlation_id={correlation_id}): {e}",
            )
            raise
    ...
    def clear_metrics(self, older_than_hours: int = 24) -> None:
        ...
        with self._lock:
            ...
            self.metrics_buffer = deque(
                [m for m in self.metrics_buffer if m.start_time >= cutoff_time],
...
def get_performance_monitor() -> PerformanceMonitor:
    ...
    if _performance_monitor is None:
        enable_detailed = os.getenv("PYDEVKIT_DETAILED_METRICS", "true").lower() in (
            "true",
            "1",
            "yes",
        )
