...
from pheno.utilities.cache.base import CacheProtocol, NullCache
...
class EnhancedVectorSearchService:
    ...
    def __init__(
        ...
        *,
        ...
        self.progressive_service = ProgressiveEmbeddingService(
            supabase_client, self.embedding_service,
        )
    ...
    async def semantic_search(
        ...
        """
        filter_key = repr(filters) if filters else None
        cache_key = (
            "semantic",
            query,
        ...
        if ensure_embeddings:
            embedding_stats = await self.progressive_service.ensure_embeddings_for_search(
                search_entities,
                limit=limit * 2,
                background=True,  # Generate a bit more than we need  # Non-blocking
            )
        ...
        if len(primary_results.results) < min(limit, 5) and ensure_embeddings:
            ...
            additional_stats = await self.progressive_service.ensure_embeddings_for_search(
                search_entities,
                limit=10,
                background=False,  # Small batch for immediate results  # Blocking
            )
        ...
        await self.cache.set(cache_key, primary_results, ttl=self.cache_ttl)
    ...
    async def hybrid_search(
        ...
        This combines semantic search (with embedding generation) and keyword search for
        the best coverage of both embedded and non-embedded content.
        """
        search_entities = entity_types or list(self.vector_search.searchable_entities.keys())
        ...
        if ensure_embeddings:
            await self.progressive_service.ensure_embeddings_for_search(
                search_entities, limit=limit * 2, background=True,
            )
        ...
        result = await self.vector_search.hybrid_search(
    ...
    async def keyword_search(
        ...
        """
        filter_key = repr(filters) if filters else None
        cache_key = ("keyword", query, limit, tuple(entity_types or []), filter_key)
        cached = await self.cache.get(cache_key)
        if cached:
            return cached

        result = await self.vector_search.keyword_search(query, limit, entity_types, filters)
        await self.cache.set(cache_key, result, ttl=self.cache_ttl)
        return result
    ...
    async def similarity_search_by_content(
        ...
        if ensure_embeddings:
            await self.progressive_service.ensure_embeddings_for_search(
                [entity_type], limit=limit * 3, background=True,
            )
        ...
        result = await self.vector_search.similarity_search_by_content(
    ...
    async def comprehensive_search(
        ...
        await self.progressive_service.ensure_embeddings_for_search(
            search_entities, limit=limit, background=True,
        )
        ...
        keyword_task = self.keyword_search(query, limit // 2, entity_types, filters)
    ...
    async def get_embedding_coverage(
        ...
        for entity_type in search_entities:
            ...
            try:
                ...
                embedded_query = self.supabase.table(table_name).select(
                ...
                logger.exception(f"Error getting coverage for {entity_type}: {e}")
