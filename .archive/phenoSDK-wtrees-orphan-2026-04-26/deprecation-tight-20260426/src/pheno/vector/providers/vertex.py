...
VERTEX_AI_AVAILABLE = importlib.util.find_spec("vertexai") is not None
vertexai = None
TextEmbeddingModel = None


def _ensure_vertexai_loaded() -> None:
    """
    Import vertexai lazily to avoid import-time crashes in constrained environments.
    """

    global vertexai, TextEmbeddingModel, VERTEX_AI_AVAILABLE
    ...
    try:
        ...
        try:
            model_cls = importlib.import_module("vertexai.language_models").TextEmbeddingModel  # type: ignore[attr-defined]
        except Exception:
            model_cls = importlib.import_module("vertexai.preview.language_models").TextEmbeddingModel  # type: ignore[attr-defined]
    except Exception as exc:
        VERTEX_AI_AVAILABLE = False
        raise RuntimeError(
            "Failed to import Vertex AI libraries. Install google-cloud-aiplatform and ensure dependencies are working.",
        ) from exc

    vertexai = vertexai_module
    TextEmbeddingModel = model_cls
...
class VertexAIEmbeddingService:
    ...
    def __init__(self, api_key: str | None = None, cache_size: int = 1000):
        ...
        if not project:
            raise RuntimeError(
                "GOOGLE_CLOUD_PROJECT (or GCP_PROJECT) must be set for Vertex AI embeddings",
            )
    ...
    def _load_persistent_cache(self):
        ...
        if self.persistent_cache_file.exists():
            try:
                ...
                with open(self.persistent_cache_file) as f:
                    data = json.load(f)
    ...
    async def generate_embedding(
        ...
        try:
            ...
            embeddings = await asyncio.to_thread(
                self._model_instance.get_embeddings, [text], output_dimensionality=768,
            )
            ...
            raise RuntimeError(f"Failed to generate embedding (Vertex AI): {e!s}")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            if uncached_texts:
                try:
                    ...
                    for idx, (original_idx, emb_obj) in enumerate(
                        zip(uncached_indices, responses, strict=False),
                    ):
                    ...
                    raise RuntimeError(f"Failed to generate batch embeddings (Vertex AI): {e!s}")
    ...
    def get_cache_stats(self) -> dict[str, int]:
        ...
        """
        return {"cache_size": len(self.cache), "cache_limit": self.cache_size}
...
# Backwards compatibility alias expected by EmbeddingProvider
VertexEmbeddingService = VertexAIEmbeddingService
