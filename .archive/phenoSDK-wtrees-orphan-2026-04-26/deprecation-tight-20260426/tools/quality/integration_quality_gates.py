...
class IntegrationQualityGates:
    ...
    def _validate_monitoring(
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if "health" in node.name.lower() and not self._has_proper_health_check(
                    node,
                ):
                    violations.append(
                        QualityGateViolation(
            ...
            for path, methods in api_spec.get("paths", {}).items():
                for method, spec in methods.items():
                    if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                        endpoint_violations = self._validate_endpoint_spec(
                            path, method, spec,
                        )
    ...
    def _has_documentation(self, node: ast.ClassDef) -> bool:
        ...
        return docstring is not None and len(docstring.strip()) > 50
...
def main()  # noqa: PLR0912, PLR0915:
    ...
    if args.validate_deployment:
        deployment_results = gates.validate_deployment_readiness(
            args.validate_deployment,
        )
