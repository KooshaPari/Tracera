    ...
    def _load_test_configs(self) -> dict[str, Any]:
        return {
            "database_tier": {
                ...
                "parallel_workers": 2,
                "command": (
                    "pytest tests/database_integration.py tests/test_services.py "
                    "-n {workers}"
                ),
            },
            "api_tier": {
                ...
                "parallel_workers": 3,
                "command": (
                    "pytest tests/test_api_endpoints.py tests/test_auth.py -n {workers}"
                ),
            },
            "external_tier": {
            ...
            "unit_tier": {
                ...
                "parallel_workers": 3,
                "command": (
                    "pytest tests/unit/ tests/utils/ tests/validation/ -n {workers}"
                ),
            },
        }

    def run_parallel_tests(self, tiers: list[str] | None = None, workers: int = 4):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run tests in parallel for fast feedback",
    )
