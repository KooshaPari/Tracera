...
app = typer.Typer(help="Launch the observability dashboard and alerting system.")
...
class ObservabilityDashboard:
    ...
    def _check_alerts(self) -> None:
        ...
        current_metrics = {
            m["name"]: m["value"] for m in self.metrics_history[-10:]
        }  # Last 10 metrics
        ...
        for alert in self.alerts:
            ...
            if (
                ...
                and time.time() - alert.last_triggered < alert.cooldown
            ):
                continue
    ...
    def _handle_file_alert(self, alert_event: AlertEvent) -> None:
        ...
        with open(alert_log_file, "a") as f:
            f.write(
                f"{datetime.fromtimestamp(alert_event.timestamp).isoformat()} - {alert_event.severity.upper()} - {alert_event.message}\n",
            )
    ...
    def _calculate_overall_health_score(self) -> float:
        ...
        if (
            "system_cpu_usage" in latest_metrics
            and latest_metrics["system_cpu_usage"] > 90
        ):
            score -= 20
        if (
            "system_memory_usage" in latest_metrics
            and latest_metrics["system_memory_usage"] > 85
        ):
            score -= 20
        ...
        if (
            "security_high_severity" in latest_metrics
            and latest_metrics["security_high_severity"] > 0
        ):
            score -= 25

        # Deduct for low kit health
        if (
            "kit_health_score" in latest_metrics
            and latest_metrics["kit_health_score"] < 70
        ):
            score -= 10
    ...
    def _generate_final_report(self) -> Path:
        ...
        console.print("📊 Generating Final Observability Report...")
        ...
        console.print("📊 Observability reports saved:")
...
def run_dashboard(
    ...
    duration: int = typer.Option(
        3600, "--duration", help="Dashboard runtime in seconds",
    ),
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Optional path to copy the final JSON report",
    ),
    json_output: bool = typer.Option(
        False, "--json", help="Emit JSON report to stdout",
    ),
    env: str = typer.Option("local", "--env", help="Environment label for context"),
    target: str = typer.Option("local", "--target", help="Deployment target"),
    verbose: bool = typer.Option(False, "--verbose", "-v"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Skip side effects"),
) -> None:
    """Launch the observability dashboard and produce structured reports."""
    ...
    dashboard = ObservabilityDashboard(str(ctx.project_root))

    try:
        dashboard.start_dashboard(duration)
    except KeyboardInterrupt:
        console.print("\n🛑 Dashboard stopped by user")
        dashboard._generate_final_report()
    except Exception as exc:
        console.print(f"❌ Error in dashboard: {exc}")
        raise typer.Exit(code=1) from exc

    if output and dashboard.last_report_path:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_bytes(dashboard.last_report_path.read_bytes())
        console.print(f"📁 Copied report to {output}")

    if json_output and dashboard.last_report:
        typer.echo(json.dumps(dashboard.last_report, indent=2))
