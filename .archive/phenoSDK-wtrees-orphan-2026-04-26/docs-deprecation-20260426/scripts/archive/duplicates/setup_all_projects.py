...
# Placeholder for missing setup_project module
class ProjectSetup:
    def __init__(self, project_name: str):
        self.project_name = project_name
        self.replacements: dict[str, str] = {}

    def set_replacements(self, **kwargs):
        self.replacements.update(kwargs)
    ...
    def setup(self):
        print(f"Setting up project: {self.project_name}")
        return True
...
def setup_project(
    ...
    setup = ProjectSetup(str(templates_dir))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Setup CI/CD for all pheno-sdk projects",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--projects",
        nargs="+",
        help="Only setup specific projects (space-separated names)",
    )
    ...
    if args.projects:
        projects_to_setup = [
            p for p in PHENO_SDK_PROJECTS if p["name"] in args.projects
        ]
