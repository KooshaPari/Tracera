...
class QualityMetricsCollector:
    ...
    def _collect_security_metrics(self) -> None:
        ...
        if bandit_file.exists():
            ...
            security_score = max(
                0,
                100 - (high_severity * 20 + medium_severity * 10 + low_severity * 2),
            )
            self.metrics.append(
                QualityMetric(
                    name="security_score",
            ...
            try:
                ...
                for func_name in functions:
                    ...
                    if match:
                        ...
                        complexity = len(
                            re.findall(
                                r"\b(if|for|while|try|except|with)\b", func_body,
                            ),
                        )
    ...
    def _collect_technical_debt_metrics(self) -> None:
        ...
        total_lines = sum(
            len(open(f, encoding="utf-8").readlines()) for f in python_files
        )
        if total_lines > 0:
        ...
        report = {
            ...
            "summary": {
                ...
                "improving_trends": len(
                    [t for t in self.trends if t.trend_direction == "improving"],
                ),
                "declining_trends": len(
                    [t for t in self.trends if t.trend_direction == "declining"],
                ),
            },
        }
    ...
    def _generate_quality_recommendations(self) -> list[str]:
        ...
        for metric in self.metrics:
            if metric.target and metric.value < metric.target:
                recommendations.append(
                    f"Improve {metric.name}: {metric.value:.1f}% (target: {metric.target}%)",
                )

            if metric.threshold and metric.value > metric.threshold:
                recommendations.append(
                    f"Address {metric.name}: {metric.value:.1f} (threshold: {metric.threshold})",
                )
        ...
        for trend in self.trends:
            if trend.trend_direction == "declining" and abs(trend.change_percent) > 10:
                recommendations.append(
                    f"Reverse declining trend in {trend.metric_name}: {trend.change_percent:.1f}%",
                )
