"""
Progressive embedding service that generates embeddings on-demand during search
operations.
"""
...
from ..providers.factory import get_embedding_service
...
class ProgressiveEmbeddingService:
    ...
    async def ensure_embeddings_for_search(
        self, entity_types: list[str], limit: int | None = None, background: bool = True,
    ) -> dict[str, int]:
            ...
            try:
                ...
                if missing_records:
                    if background:
                        ...
                        results[entity_type] = (
                            f"Generating {len(missing_records)} embeddings in background"
                        )
                    else:
                        # Generate embeddings synchronously (blocks search)
                        generated = await self._generate_embeddings_for_records(
                ...
                logger.exception(f"Error ensuring embeddings for {entity_type}: {e}")
                results[entity_type] = f"Error: {e!s}"

        return results

    async def generate_embedding_on_demand(
        self, table_name: str, record_id: str, record_data: dict[str, Any] | None = None,
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            await self._update_record_embedding(
                table_name, record_id, embedding_vector, record_data,
            )
            timings["db_update"] = time.time() - t0

            logger.info(f"Generated embedding for {record_key} ({len(content)} chars)")
            return {"embedding": embedding_vector, "timings": timings}

        except Exception as e:
            logger.exception(f"Failed to generate embedding for {record_key}: {e}")
            return None
        finally:
    ...
    async def _find_records_without_embeddings(
        ...
        try:
            ...
            logger.exception(f"Error finding records without embeddings in {table_name}: {e}")
            return []
    ...
    async def _generate_embeddings_for_records(
        ...
        for record in records:
            ...
            try:
                embedding = await self.generate_embedding_on_demand(table_name, record_id, record)
                if embedding:
                    generated_count += 1
                ...
                logger.exception(f"Error generating embedding for {table_name}:{record_id}: {e}")
                continue

        logger.info(f"Generated {generated_count}/{len(records)} embeddings for {table_name}")
        return generated_count

    async def _fetch_record(self, table_name: str, record_id: str) -> dict[str, Any] | None:
        """
        Fetch a single record from the database.
        """
        try:
            result = (
                self.supabase.table(table_name).select("*").eq("id", record_id).single().execute()
            )
            return result.data if result.data else None
        except Exception as e:
            logger.exception(f"Error fetching record {table_name}:{record_id}: {e}")
            return None

    def _extract_content(self, record: dict[str, Any], table_name: str) -> str | None:
        """
        ...
        try:
            ...
            if last_error:
                ...
                if "permission denied" in error_msg.lower():
                    logger.warning(
                        f"RLS prevented direct update for {table_name}:{record_id}, trying direct table access",
                    )
                else:
                ...
                try:
                    update_data = {config["embedding_column"]: embedding}
                    update_query = (
                        self.supabase.table(table_name).update(update_data).eq("id", record_id)
                    )
                    ...
                    logger.exception(
                        f"❌ Both RPC and direct update failed for {table_name}:{record_id}: {direct_error}",
                    )
                    return False

        except Exception as e:
            logger.exception(
                f"❌ Unexpected error updating embedding for {table_name}:{record_id}: {e}",
            )
            return False

    def _get_table_name(self, entity_type: str) -> str | None:
        """
