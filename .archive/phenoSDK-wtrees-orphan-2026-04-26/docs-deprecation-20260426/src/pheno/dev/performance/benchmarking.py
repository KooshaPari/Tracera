...
class Benchmarker:
    ...
    def compare_providers(self, benchmark_keys: list[str] | None = None) -> dict[str, Any]:
    ...
    def get_fastest_provider(self, min_success_rate: float = 0.8) -> str | None:
        ...
        with self.monitor._lock:
            for benchmark in self.provider_benchmarks.values():
                if (
                    benchmark.avg_response_time < best_time
                    and benchmark.success_rate >= min_success_rate
                ):
