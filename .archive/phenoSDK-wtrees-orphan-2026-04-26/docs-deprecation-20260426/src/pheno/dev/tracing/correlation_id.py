...
This module provides correlation ID functionality for tracking requests across multiple
components, tools, and providers in the Zen MCP Server.
"""
        ...
        if hasattr(handler, "formatter") and handler.formatter:
            ...
            if "correlation_id" not in current_format:
                ...
                new_format = current_format.replace(
                    "%(message)s", "%(message)s [correlation_id=%(correlation_id)s]",
                )
