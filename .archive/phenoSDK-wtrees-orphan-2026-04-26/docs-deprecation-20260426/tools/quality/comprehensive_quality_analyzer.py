...
class ComprehensiveQualityAnalyzer:
    ...
    def run_analysis(
        ...
        for tool_name, tool_config in self.quality_tools.items():
            ...
            if report.status == "success":
                print(
                    f"  ✅ {tool_config['description']} completed ({report.duration:.2f}s)",
                )
            else:
    ...
    def _run_quality_tool(
        ...
        try:
            # Prepare command
            cmd = (
                ["python3", tool_config["script"]] + tool_config["args"] + [target_path]
            )
    ...
    def _generate_unified_report(self) -> dict[str, Any]:
        ...
        successful_tools = sum(
            1 for report in self.reports if report.status == "success"
        )
    ...
    def _print_summary(self, report: dict[str, Any]) -> None:
        ...
        print(
            f"Tools executed: {metadata['successful_tools']}/{metadata['total_tools']}",
        )
        print()
...
def main():
    ...
    if args.tool and args.tool in analyzer.quality_tools:
        ...
        report = analyzer._run_quality_tool(
            args.tool,
            tool_config,
            args.path,
            args.output,
