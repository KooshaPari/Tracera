...
class PIIScanner:
    ...
    def __init__(
        ...
        *,
        ...
        presidio_analyzer: AnalyzerEngine | None = None,
        ...
        Args:
            ...
            use_presidio: Enable Presidio analyzer integration (requires presidio-analyzer)
            presidio_analyzer: Optional AnalyzerEngine instance (useful for testing)
            presidio_language: Language code passed to Presidio analyzer
        """
        self.patterns = {
        ...
        if use_presidio:
            ...
            elif AnalyzerEngine is not None:
                self._presidio = AnalyzerEngine()
            else:  # pragma: no cover - optional dependency guard
                raise ImportError(
                    "presidio-analyzer is not installed. Install it or disable use_presidio.",
                )

    def detect(self, text: str, pattern_names: set[str] | None = None) -> list[dict[str, any]]:
        """Detect PII in text.
    ...
    def _detect_with_presidio(self, text: str) -> list[dict[str, any]]:
        ...
        try:
            results = self._presidio.analyze(text=text, language=self._presidio_language)
        ...
        for result in results or []:
            # Guard against unexpected result types
            start = getattr(result, "start", None)
            end = getattr(result, "end", None)
            entity = getattr(result, "entity_type", "PRESIDIO")
...
def detect_pii(
    ...
    **scanner_kwargs: any,
    ...
    """
    scanner = PIIScanner(custom_patterns=patterns, **scanner_kwargs)
