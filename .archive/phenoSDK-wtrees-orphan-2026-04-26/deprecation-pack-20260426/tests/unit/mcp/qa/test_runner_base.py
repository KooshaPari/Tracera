...
try:
    from pheno.mcp.qa.testing.logging_config import QuietLogger
except ImportError:
...
# Import from pheno.mcp.qa.core (shared components)
try:
    from pheno.mcp.qa.reporters import ConsoleReporter, TestReporter

    from ..cache import TestCache
    from ..test_registry import get_test_registry
except ImportError:
    # Fallback imports for standalone use
    from pheno.mcp.qa.core.cache import TestCache
    from pheno.mcp.qa.core.test_registry import get_test_registry
    from pheno.mcp.qa.reporters import ConsoleReporter, TestReporter
...
class BaseTestRunner(ABC):
    ...
    def __init__(
        ...
        reporters: list[TestReporter] | None = None,
        ...
        self._semaphore: asyncio.Semaphore | None = None  # Concurrency control
    ...
    def _extract_base_url(self) -> str:
        ...
        if hasattr(client, "_client") and hasattr(client._client, "mcp_url"):
    ...
    async def _run_single_test_with_worker(
        self, test_name: str, test_info: dict[str, Any], pbar: Any | None = None,
    ) -> None:
        """
        ...
        async with self._semaphore:
            ...
            try:
                ...
                # Log worker error
                error_msg = f"Test {test_name} failed in worker {worker_id}: {e!s}"
                with self._worker_lock:
        ...
        try:
            ...
            if self.verbose:
                test_result = await asyncio.wait_for(
                    test_func(self.client_adapter), timeout=test_timeout,
                )
            else:
                with QuietLogger():
                    test_result = await asyncio.wait_for(
            ...
            if self.cache_instance:
                self.cache_instance.record(
                    test_name, tool_name, "timeout", duration_ms / 1000, error,
                )
