...
def main() -> None:
        ...
        if isinstance(sections, dict):
            for section, value in sections.items():
                if isinstance(value, list):
                    for entry in value:
                        print(f"  - [{section}] {entry['line']} -> {entry['replace_with']}")
    ...
    print("Next steps:")
    print(
        "  1. Migrate code into the layered `pheno.*` modules per PHEN_HEXAGONAL_MIGRATION_PLAN.md.",
    )
    print("  2. Use this manifest to update packaging files and run a codemod for imports.")
    print(
        "  3. Verify with `python tools/generate_phen_codemap.py` after each batch to ensure counts drop.",
    )
