...
class WorkflowConsolidator:
    ...
    def _categorize_workflows(self) -> dict[str, list[str]]:
        ...
        for workflow_name in self.workflows:
            ...
            if "quality" in workflow_name.lower() or "lint" in workflow_name.lower():
                ...
                "release" in workflow_name.lower() or "version" in workflow_name.lower()
            ):
    ...
    def _create_consolidation_plan(
        ...
        consolidated_workflows = plan["consolidated_workflows"]
        if isinstance(consolidated_workflows, dict):
            main_workflow: dict[str, Any] = consolidated_workflows[
                "consolidated-ci.yml"
            ]
            includes_raw = main_workflow.get("includes", [])
            replaces_raw = main_workflow.get("replaces", [])
            workflows_to_remove_raw = plan.get("workflows_to_remove", [])
            workflows_to_keep_raw = plan.get("workflows_to_keep", [])
            workflows_to_modify_raw = plan.get("workflows_to_modify", [])

            # Ensure all are lists
            includes: list[str] = includes_raw if isinstance(includes_raw, list) else []
            replaces: list[str] = replaces_raw if isinstance(replaces_raw, list) else []
            workflows_to_remove: list[str] = (
                workflows_to_remove_raw
                if isinstance(workflows_to_remove_raw, list)
                else []
            )
            workflows_to_keep: list[str] = (
                workflows_to_keep_raw if isinstance(workflows_to_keep_raw, list) else []
            )
            workflows_to_modify: list[str] = (
                workflows_to_modify_raw
                if isinstance(workflows_to_modify_raw, list)
                else []
            )
        else:
            return plan
    ...
    def _generate_migration_steps(
        ...
        if consolidation_plan["workflows_to_remove"]:
            steps.append(
                f"6. Remove {len(consolidation_plan['workflows_to_remove'])} workflows: {', '.join(consolidation_plan['workflows_to_remove'])}",
            )

        if consolidation_plan["workflows_to_keep"]:
            steps.append(
                f"7. Keep {len(consolidation_plan['workflows_to_keep'])} workflows: {', '.join(consolidation_plan['workflows_to_keep'])}",
            )
...
def main():
    ...
    parser.add_argument(
        "--analyze-only",
        action="store_true",
        help="Only analyze, don't create archive",
    )
    ...
    print(
        f"  To consolidate: {len(analysis['consolidation_plan']['workflows_to_remove'])}",
    )
