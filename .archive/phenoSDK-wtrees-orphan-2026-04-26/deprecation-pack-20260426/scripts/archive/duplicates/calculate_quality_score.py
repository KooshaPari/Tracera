...
class QualityScoreCalculator:
        ...
        for pattern in report_patterns:
            for report_file in reports_path.glob(pattern):
                try:
                    ...
                    print(
                        f"Warning: Could not parse {report_file}: {e}",
                        file=sys.stderr,
                    )
    ...
    def _generate_recommendations(
        ...
        if scores["violations"] < 70:
            recommendations.append(
                "⚠️ High Priority: Address code violations to improve maintainability",
            )

        if scores["warnings"] < 60:
            recommendations.append(
                "📝 Medium Priority: Resolve warnings to improve code quality",
            )

        if scores["coverage"] < 80:
            recommendations.append(
                "🧪 Testing: Increase test coverage to improve reliability",
            )
