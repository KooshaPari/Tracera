...
| API origin | Rich gateway default resolves to `http://127.0.0.1:18000/`; no implicit Pages/Vercel URL | pending |
...
| SPA delivery | Local desktop/web runtime serves the rich `dist` output through the `18000` gateway; bundled legacy `18081` is explicit opt-in only | pending |
