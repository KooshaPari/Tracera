...
`infisical.yml` will expose the same manual/reusable `runner` input and use `${{ inputs.runner || 'ubuntu-latest' }}`, defaulting to `ubuntu-latest` for push, pull request, and reusable calls. Secret values and the five-minute timeout remain unchanged.
