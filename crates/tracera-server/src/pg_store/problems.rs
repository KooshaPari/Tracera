...
use crate::store::{ListParams, Problem, StoreError, StoreResult};

pub(super) async fn list_problems(
    pool: &PgPool,
    project_id: String,
...
pub(super) async fn create_problem(
    pool: &PgPool,
    id: String,
    project_id: String,
...
pub(super) async fn count_problems(pool: &PgPool, project_id: String) -> StoreResult<i64> {
    let row = sqlx::query(
...
pub(super) async fn count_problems_filtered(
    pool: &PgPool,
    project_id: String,
    status_filter: Option<String>,
) -> StoreResult<i64> {
...
pub(super) async fn dashboard_status_counts(
    pool: &PgPool,
) -> StoreResult<Vec<(String, String, i64)>> {
    let rows = sqlx::query(
        "SELECT project_id::text AS project_id, status, COUNT(*)::bigint AS cnt
         FROM problems
    ...
    Ok(rows
        ...
        .map(|row| {
            (
                ...
                row.try_get("cnt").unwrap_or_default(),
            )
        })
        .collect())
}
...
// -----------------------------------------------------------------------
pub(super) fn pg_row_to_problem(r: sqlx::postgres::PgRow) -> Problem {
