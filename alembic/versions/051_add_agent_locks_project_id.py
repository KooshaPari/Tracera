...
down_revision: Union[str, None] = "050_add_processes_if_not_exists"
...
def upgrade() -> None:
    ...
    if r is not None:
        ...
        op.execute(
            f"CREATE INDEX IF NOT EXISTS idx_locks_project_item ON {qual} (project_id, item_id)"
        )
        return
    ...
    r2 = conn.execute(
        text(
            ...
            "WHERE table_name = 'agent_locks' LIMIT 1"
        )
    ).fetchone()
    if r2 is None:
        return
    ...
    op.execute(
        f"ALTER TABLE {qual} ADD COLUMN IF NOT EXISTS project_id UUID "
        f"REFERENCES {proj_ref} ON DELETE CASCADE"
    )
    op.execute(
    ...
    null_count = conn.execute(
        text(f"SELECT COUNT(*) FROM {qual} WHERE project_id IS NULL")
    ).scalar()
    if null_count == 0:
        op.execute(
            f"ALTER TABLE {qual} ALTER COLUMN project_id SET NOT NULL"
        )
    op.execute(
        f"CREATE INDEX IF NOT EXISTS idx_locks_project_item ON {qual} (project_id, item_id)"
    )
