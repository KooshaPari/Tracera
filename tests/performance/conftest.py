...
def measure_time(name: str, perf_tracker=None, threshold_ms: Optional[float] = None):
    ...
    try:
        ...
        if threshold_ms is not None and elapsed_ms > threshold_ms:
            pytest.fail(
                f"{name} exceeded threshold: {elapsed_ms:.2f}ms > {threshold_ms:.2f}ms"
            )
...
def assert_performance(elapsed_ms: float, threshold_ms: float, operation_name: str) -> None:
    ...
    assert elapsed_ms <= threshold_ms, (
        f"{operation_name} performance degradation: "
        f"{elapsed_ms:.2f}ms > {threshold_ms:.2f}ms threshold"
    )
