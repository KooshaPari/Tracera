...
def create_item(project_path, title, view, item_type, description=None, status="todo", priority="medium", owner=None, parent_id=None):
    """Create an item using rtm CLI."""
    cmd = ["rtm", "item", "create", title, "--view", view, "--type", item_type, "--status", status, "--priority", priority]
    if description:
...
def populate_demo_project():
    ...
    stories = [
        ("Password Reset Flow", "As a user, I want to reset my password via email so that I can regain access to my account if I forget it.", "high"),
        ("Two-Factor Authentication", "As a user, I want to enable 2FA so that my account is more secure.", "medium"),
        ("Social Login", "As a user, I want to login with Google/GitHub so that I don't need to remember another password.", "medium"),
        ("Session Management", "As a user, I want to see my active sessions and logout from devices so that I can manage my account security.", "low"),
    ]
    ...
    if epic2_id:
        ...
        catalog_stories = [
            ("Product Search", "As a customer, I want to search for products by name, brand, or category so that I can quickly find what I'm looking for.", "high"),
            ("Advanced Filters", "As a customer, I want to filter products by price, rating, brand, and features so that I can narrow down my options.", "high"),
            ("Product Recommendations", "As a customer, I want to see recommended products based on my browsing history so that I can discover new items I might like.", "medium"),
            ("Product Reviews & Ratings", "As a customer, I want to read reviews and see ratings before purchasing so that I can make informed decisions.", "high"),
            ("Wishlist", "As a customer, I want to save products to a wishlist so that I can purchase them later.", "medium"),
        ]
    ...
    if epic3_id:
        ...
        checkout_stories = [
            ("Add to Cart", "As a customer, I want to add products to my cart so that I can purchase multiple items together.", "critical"),
            ("Cart Management", "As a customer, I want to update quantities, remove items, and see cart total so that I can manage my purchase.", "high"),
            ("Guest Checkout", "As a customer, I want to checkout without creating an account so that I can purchase quickly.", "high"),
            ("Multiple Payment Methods", "As a customer, I want to pay with credit card, PayPal, or Apple Pay so that I can use my preferred payment method.", "high"),
            ("Shipping Address Management", "As a customer, I want to save multiple shipping addresses so that I can ship to different locations.", "medium"),
            ("Order Confirmation", "As a customer, I want to receive order confirmation with tracking number so that I know my order was placed successfully.", "critical"),
        ]
...
def populate_frontend_project():
    ...
    if not (Path(project_path) / ".trace" / "project.yaml").exists():
        ...
        run_cmd(["rtm", "init", "--name", "Frontend Application", "--description", "React-based frontend application with modern UI components", "--path", project_path])
    ...
    if epic1_id:
        ...
        component_stories = [
            ("Button Component", "Create a reusable Button component with variants (primary, secondary, danger) and sizes (sm, md, lg)", "high"),
            ("Form Input Components", "Create Input, Textarea, Select, and Checkbox components with validation states", "high"),
    ...
    if epic2_id:
        ...
        auth_stories = [
            ...
            ("Registration Page", "Build registration form with password strength indicator and terms acceptance", "high"),
...
def populate_backend_project():
    ...
    if not (Path(project_path) / ".trace" / "project.yaml").exists():
        ...
        run_cmd(["rtm", "init", "--name", "Backend API", "--description", "RESTful API backend with authentication, business logic, and data persistence", "--path", project_path])
