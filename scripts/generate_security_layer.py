...
# Storage for linking
created_items = {k: [] for k in stats.keys()}


async def create_item(session, item_type, ext_id, title, desc, status='todo',
                     priority='medium', metadata=None):
...
async def generate_vulnerabilities(session):
    ...
    for i, (title, cvss, cwe, component) in enumerate(sql_vulns, 1):
        await create_item(
            session, 'security_vulnerability', f'VULN-{i:03d}',
            title,
            f"{title}: {component} vulnerable to SQL injection attacks. CVSS {cvss}.",
            'open', 'critical' if cvss >= 9 else 'high' if cvss >= 7 else 'medium',
    ...
    for i, (title, cvss, component) in enumerate(xss_vulns, 13):
        await create_item(
            ...
            'open', 'high' if cvss >= 7 else 'medium',
...
async def generate_security_controls(session):
    ...
    for i in range(1, 101):
        ...
        await create_item(
            session, 'security_control', f'CTRL-{i:03d}',
            title, desc, 'implemented' if i <= 50 else 'planned',
...
async def generate_threat_models(session):
    ...
    for i in range(1, 61):
        ...
        await create_item(
            session, 'threat_model', f'THREAT-{i:03d}',
            title, desc, 'identified',
            'critical' if i <= 10 else 'high' if i <= 30 else 'medium',
            {'stride_category': ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'][i % 6]}
        )
...
async def generate_security_tests(session):
    ...
    for i in range(1, 91):
        ...
        await create_item(
            session, 'security_test', f'SECTEST-{i:03d}',
            title, desc, 'passed' if i <= 60 else 'todo',
            'high' if i <= 30 else 'medium',
...
async def generate_encryption_requirements(session):
    ...
    for i in range(1, 51):
        ...
        await create_item(
            session, 'encryption_requirement', f'ENCRYPT-{i:03d}',
            title, desc, 'implemented' if i <= 30 else 'planned',
...
async def generate_access_controls(session):
    ...
    for i in range(1, 71):
        ...
        await create_item(
            session, 'access_control', f'AC-{i:03d}',
            title, desc, 'active',
