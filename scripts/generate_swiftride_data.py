...
class ItemGenerator:
    ...
    def add_item(self, item_type: str, title: str, description: str, status: str = 'active',
                 priority: int = 2, metadata: dict = None, tags: list = None, parent_id: str = None) -> str:
    ...
    def add_link(self, source_id: str, target_id: str, link_type: str, metadata: dict = None):
...
def generate_swiftride_data():
    ...
    objectives = []
    obj_data = [
        ("Achieve Market Leadership", "Become #1 ride-sharing platform in top 50 US cities with 40% market share",
         {"target_date": "2027-12-31", "kpi": "market_share", "target": 40}),
        ("Revenue Growth to $5B ARR", "Scale annual recurring revenue to $5 billion",
         {"target_date": "2027-12-31", "kpi": "arr", "target": 5000000000}),
        ("Driver Satisfaction Above 4.5", "Maintain driver satisfaction at 4.5+ stars",
         {"target_date": "2026-12-31", "kpi": "driver_satisfaction", "target": 4.5}),
        ("Carbon Neutral by 2028", "Achieve 100% carbon neutral rides",
         {"target_date": "2028-12-31", "kpi": "carbon_neutrality", "target": 100}),
        ("Zero Safety Fatalities", "Eliminate all fatal accidents through technology and training",
         {"target_date": "2027-12-31", "kpi": "fatality_rate", "target": 0}),
        ("Expand to 100 Cities", "Launch operations in 100 major metropolitan areas",
         {"target_date": "2027-06-30", "cities_current": 50, "cities_target": 100}),
        ("Driver Fleet to 1M", "Grow active driver base to 1 million",
         {"target_date": "2028-12-31", "current": 125000, "target": 1000000}),
        ("Customer LTV $2000", "Increase customer lifetime value through retention",
         {"target_date": "2026-12-31", "current": 850, "target": 2000}),
    ]

    for title, desc, meta in obj_data:
        obj_id = gen.add_item('business_objective', title, desc, 'in_progress', 1, meta,
                             ['strategic', 'growth'])
    ...
    for title, desc, unit, freq, target in kpi_data:
        kpi_id = gen.add_item('kpi', title, desc, 'active', 1,
    ...
    for title, desc, size, growth, spend in segment_data:
        seg_id = gen.add_item('market_segment', title, desc, 'active', 1,
    ...
    for title, desc, ptype, age, income in persona_data:
        persona_id = gen.add_item('persona', title, desc, 'active', 1,
    ...
    for title, desc, priority in rule_data:
        rule_id = gen.add_item('business_rule', title, desc, 'active', priority,
    ...
    for title, desc, jurisdiction, category in comp_data:
        comp_id = gen.add_item('compliance', title, desc, 'active', 1,
    ...
    for title, desc, percent, priority in rev_data:
        rev_id = gen.add_item('revenue_model', title, desc, 'active', priority,
                             {"contribution_percent": percent}, ['revenue', 'business-model'])
    ...
    for title, desc, priority in price_data:
        price_id = gen.add_item('pricing_strategy', title, desc, 'active', priority,
                               {"dynamic": title.find("Dynamic") != -1}, ['pricing', 'strategy'])
    ...
    epics = []
    epic_data = [
        ("Driver Onboarding & Verification", "Complete driver signup, background checks, vehicle inspection", "driver-platform"),
    ...
    for title, desc, team in epic_data:
        epic_id = gen.add_item('epic', title, desc, 'in_progress', 1,
                              {"team": team, "effort": "large"}, ['epic', team])
    ...
    for i in range(100):
        ts_id = gen.add_item(
            ...
            f"TypeScript React component/module for frontend",
            'active',
            2,
    ...
    print(f"    Created {len(unit_tests)} unit tests, {len(integration_tests)} integration tests, {len(e2e_tests)} e2e tests...")
    ...
    for i in range(30):
        ...
        iac_title = f"IaC: {['Kubernetes Manifests', 'Terraform', 'Helm Charts', 'Docker Compose'][i % 4]} - Infra {i+1}"
    ...
    adrs = []
    adr_topics = [
        "Microservices Architecture", "Event-Driven Communication", "PostgreSQL Database",
        "Redis Caching Strategy", "NATS Message Broker", "Kubernetes Orchestration",
        "RESTful API Design", "GraphQL vs REST", "Authentication with JWT",
        "Payment Provider Selection", "Geospatial Indexing", "Real-time Location Updates"
    ]
    for i in range(30):
        adr_id = gen.add_item(
            'adr',
    ...
    guides = []
    guide_topics = [
        "Getting Started as Rider", "Getting Started as Driver", "Request Your First Ride",
        "Become a Driver", "Schedule a Ride", "Add Payment Method", "Rate Your Experience",
        "Corporate Account Setup", "Understanding Surge Pricing", "Safety Features Guide"
    ]
    for i in range(20):
    ...
    with open('/Users/kooshapari/temp-PRODVERCEL/485/kush/trace/scripts/enhance_swiftride_full.sql', 'w') as f:
        f.write(sql_content)
    ...
    print(f"Execute with: psql postgresql://tracertm:tracertm_password@localhost:5432/tracertm -f scripts/enhance_swiftride_full.sql")
    print(f"Then run: ./scripts/quick_fix_graph.sh")
    print(f"{'='*80}\n")
