...
def _is_only_200_noise(phrase: str) -> bool:
    ...
    if not lines:
        return True
    if all(FILTER_200_INLINE.search(ln) or FILTER_200.match(ln) for ln in lines):
        return True
    return False
    ...
    def run_py() -> None:
        _run_service(
            ...
            [
                ...
                "-m", "uvicorn", "src.tracertm.api.main:app",
                "--reload", "--host", "0.0.0.0", "--port", "8000",
                "--reload-exclude", "ARCHIVE", "--reload-exclude", "CONFIG",
            ],
            env,
            phrase_queue,
            processes,
