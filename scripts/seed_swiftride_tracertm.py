...
def main() -> int:
    ...
    ap.add_argument("--account-id", default=os.environ.get("ACCOUNT_ID"), help="Account UUID for projects (or set ACCOUNT_ID)")
    ap.add_argument("--project-id", default=os.environ.get("PROJECT_ID"), help="Existing project UUID to extend (items/links only; no project INSERT). Set PROJECT_ID=uuid or --project-id")
    ...
    if not extend_existing and not account_id:
        ...
        print("-- WARNING: ACCOUNT_ID not set; using placeholder. Set ACCOUNT_ID=your-uuid or --account-id", file=sys.stderr)
    ...
    try:
        ...
        if extend_existing:
            out.write("-- Target project (extend only): " + project_uuid + "\n")
        out.write("SET search_path TO tracertm;\n\n")
        out.write("-- Delete in order: links reference items and graph; items reference project\n")
        out.write("DELETE FROM links WHERE project_id = '" + project_uuid + "'::uuid;\n")
        out.write("DELETE FROM graph_nodes WHERE project_id = '" + project_uuid + "'::uuid;\n")
        out.write("DELETE FROM items WHERE project_id = '" + project_uuid + "'::uuid;\n")
        out.write("DELETE FROM graphs WHERE project_id = '" + project_uuid + "'::uuid;\n")
        if not extend_existing:
            out.write("DELETE FROM projects WHERE id = '" + project_uuid + "'::uuid;\n\n")
            if account_id:
                out.write("INSERT INTO projects (id, account_id, name, description, metadata, created_at, updated_at)\n")
                out.write("VALUES (" + sql_quote(project_uuid) + "::uuid, " + sql_quote(account_id) + ", " + sql_quote(project["name"]) + ", " + (sql_quote(desc_from_meta) if desc_from_meta else "NULL") + ", '" + meta_str + "'::jsonb, NOW(), NOW());\n\n")
            else:
                out.write("INSERT INTO projects (id, name, description, metadata, created_at, updated_at)\n")
                out.write("VALUES (" + sql_quote(project_uuid) + "::uuid, " + sql_quote(project["name"]) + ", " + (sql_quote(desc_from_meta) if desc_from_meta else "NULL") + ", '" + meta_str + "'::jsonb, NOW(), NOW());\n\n")
        else:
            out.write("\n")
        ...
        out.write("INSERT INTO items (id, project_id, title, description, type, status, priority, parent_id, metadata, created_at, updated_at)\nVALUES\n")
        for i, it in enumerate(items):
            ...
            out.write(
                "  (" + sql_quote(u) + ", " + sql_quote(project_uuid) + ", "
                + sql_quote(it["title"]) + ", " + sql_quote(it["description"]) + ", "
                + sql_quote(it["item_type"]) + ", " + sql_quote(it["status"]) + ", "
                + str(pri) + ", " + parent_uuid + ", '" + meta_str + "'::jsonb, NOW(), NOW())" + comma + "\n"
            )
        out.write(";\n\n")

        # Default graph for the project (required for links.graph_id)
        graph_id = str(uuid.uuid4())
        out.write("INSERT INTO graphs (id, project_id, name, graph_type, description, root_item_id, graph_metadata, created_at, updated_at)\n")
        out.write("VALUES (" + sql_quote(graph_id) + ", " + sql_quote(project_uuid) + "::uuid, 'Trace graph', 'trace', NULL, NULL, '{}'::jsonb, NOW(), NOW());\n\n")
        ...
        out.write("INSERT INTO links (id, project_id, graph_id, source_item_id, target_item_id, link_type, link_metadata, created_at, updated_at)\nVALUES\n")
        for i, (link_uuid, src, tgt, link_type, meta) in enumerate(valid_links):
            meta_esc = meta.replace("'", "''")
            comma = "," if i < len(valid_links) - 1 else ""
            out.write("  (" + sql_quote(link_uuid) + ", " + sql_quote(project_uuid) + "::uuid, " + sql_quote(graph_id) + ", " + sql_quote(src) + ", " + sql_quote(tgt) + ", " + sql_quote(link_type) + ", '" + meta_esc + "'::jsonb, NOW(), NOW())" + comma + "\n")
        out.write(";\n")
    finally:
