...
def main() -> None:
    ...
    try:
        ...
        for project in projects:
            ...
            link_set = {
                (row["graph_id"], row["source_item_id"], row["target_item_id"], row["link_type"]) for row in existing_links
            }
            ...
            def ensure_graph_node(graph_id: str | None, item_id: str):
                if not graph_id:
                    return
                key = (graph_id, item_id)
                if key in graph_node_set:
                    return
                graph_node_set.add(key)
                conn.cursor().execute(
                    """
            ...
            def insert_link(graph_id: str | None, source_id: str, target_id: str, link_type: str):
            ...
            if db_graph_id:
                for domain, tables in by_view_domain.get("database", {}).items():
