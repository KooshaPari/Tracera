...
PROJECT_DOMAINS = [
    "E-Commerce Platform", "Healthcare Management System", "Financial Trading Platform",
    "IoT Device Management", "Content Management System", "Learning Management System",
    "Customer Relationship Management", "Supply Chain Management", "Project Management Tool",
    "Social Media Platform", "Video Streaming Service", "Cloud Infrastructure",
    "Mobile Banking App", "Real Estate Platform", "Food Delivery Service",
    "Ride Sharing App", "Fitness Tracking App", "Weather Monitoring System",
    "Smart Home Automation", "Blockchain Wallet", "AI Model Training Platform",
    "Data Analytics Dashboard", "API Gateway", "Microservices Orchestration",
    "DevOps Pipeline", "Security Monitoring", "Compliance Management",
    "Document Management", "Collaboration Tool", "Code Review Platform",
    "Test Automation Framework", "Performance Testing", "Load Balancing",
    "Database Migration Tool", "Backup & Recovery", "Disaster Recovery",
    "Network Monitoring", "Log Aggregation", "Error Tracking",
    "Feature Flagging", "A/B Testing", "User Analytics",
    "Marketing Automation", "Email Campaign", "Customer Support",
    "Inventory Management", "Order Processing", "Payment Gateway",
    "Shipping Integration", "Tax Calculation", "Reporting System"
]
...
OWNERS = [
    "product_team", "backend_dev", "frontend_dev", "qa_team", "devops_team",
    "security_team", "data_team", "mobile_team", "design_team", "api_team"
]

ITEM_TYPES_BY_VIEW = {
    "requirements": ["functional_requirement", "non_functional_requirement", "business_rule", "user_story", "acceptance_criteria"],
    "features": ["feature", "epic", "user_story", "enhancement", "bug_fix"],
    "code": ["python_file", "typescript_file", "javascript_file", "go_file", "rust_file", "java_file", "service", "module", "component"],
...
LINK_TYPES = [
    "implements", "tests", "depends_on", "exposes", "uses", "validates",
    "refines", "decomposes", "related_to", "blocks", "replaces", "migrates_to"
]
...
def generate_item_title(view: str, item_type: str, index: int) -> str:
    ...
    if view == "requirements":
        ...
        extensions = {"python_file": ".py", "typescript_file": ".ts", "javascript_file": ".js",
                     "go_file": ".go", "rust_file": ".rs", "java_file": ".java"}
...
def generate_metadata(view: str, item_type: str) -> dict:
    ...
    if view == "requirements":
        base_metadata.update({
            "acceptance_criteria": [
                f"Criterion {i}: Must meet standard {random.randint(1, 10)}"
                for i in range(1, random.randint(3, 6))
            ],
            "stakeholders": random.sample(["product_manager", "tech_lead", "qa_lead", "security_team"], random.randint(2, 4)),
        ...
        base_metadata.update({
            ...
            "last_reviewed": (datetime.now(timezone.utc) - timedelta(days=random.randint(1, 90))).isoformat()
        })
    elif view == "deployment":
        base_metadata.update({
    ...
    for i in range(count):
        ...
        project = Project(
            ...
            project_metadata={
                ...
                "tech_stack": random.sample(["Python", "TypeScript", "React", "PostgreSQL", "Docker", "Kubernetes"], random.randint(3, 6)),
                "start_date": (datetime.now(timezone.utc) - timedelta(days=random.randint(30, 365))).isoformat(),
...
def create_links(session: Session, project_ids: List[str], graph_map: Dict[str, str],
                 project_items: Dict[str, List[Item]], links_per_project: int = 50) -> None:
    ...
    for project_id in project_ids:
        ...
        for i in range(agents_per_project):
            ...
            agent = Agent(
                ...
                last_activity_at=(datetime.now(timezone.utc) - timedelta(minutes=random.randint(1, 1440))).isoformat()
            )
            session.add(agent)
            agents.append(agent)
            agent_count += 1
...
def create_events(session: Session, project_ids: List[str], project_agents: Dict[str, List[Agent]],
                 project_items: Dict[str, List[Item]], events_per_project: int = 20) -> None:
    """Create agent events."""
        ...
        for _ in range(events_per_project):
            ...
            event = AgentEvent(
                ...
                event_data={
                    ...
                    "timestamp": (datetime.now(timezone.utc) - timedelta(minutes=random.randint(1, 10080))).isoformat(),
                    "details": {
