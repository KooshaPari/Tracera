...
def export_project(conn, project_id, out_dir):
    project = fetch_all(conn, "select id, name, description, project_metadata from projects where id = %(pid)s", {'pid': project_id})
    if not project:
        return
    project = project[0]
    ...
    graph_nodes = fetch_all(conn, """
        select graph_id, item_id
        from graph_nodes
        where project_id = %(pid)s
    """, {'pid': project_id})

    links = fetch_all(conn, """
    ...
    node_kinds = []
    if node_kind_ids:
        node_kinds = fetch_all(conn, "select id, name, description, kind_metadata, project_id from node_kinds where id = any(%(ids)s)", {'ids': node_kind_ids})
    ...
    file_path = os.path.join(out_dir, f"{project_id}.graphs.json")
...
def main():
    ...
    out_dir = os.path.join(EXPORT_ROOT, f"graphs_only_snapshot_{timestamp}")
