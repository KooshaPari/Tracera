...
def extract_issues(log_path: Path, suite_name: str, cwd: Path) -> list[tuple[str, int | None, str]]:
    ...
    for raw_line in text.splitlines():
        ...
        if not line_stripped or line_stripped.startswith("Running ") or line_stripped.startswith("["):
            continue
        # Skip make / shell noise
        if line_stripped.startswith("make[") or line_stripped.startswith("$ ") or "*** [" in line:
            continue
        for pat, kind in PATTERNS:
            ...
            if kind == "go":
                ...
                break
            if kind == "go_fail":
                issues.append((f"backend (Go tests)", None, f"FAIL: {g[0]}"))
                break
            if kind == "ruff":
...
def extract_by_file_from_log_text(
    text: str, cwd: Path, suite_name: str
    ...
    for raw_line in text.splitlines():
        ...
        if not line or line.startswith("make[") or line.startswith("$ ") or "*** [" in line:
            continue
        for pat in FILE_LINE_PATTERNS:
            m = pat.match(line)
            if not m:
...
def print_action_plan_by_log(
    failed_suites: list[str], with_content: list[tuple[str, Path]], cwd: Path
) -> None:
