...
def insert_link(conn, project_id: str, graph_id: str | None, source_id: str, target_id: str, link_type: str, ensure_nodes: bool = False):
...
def main():
    ...
    try:
        ...
        for project in projects:
            ...
            for domain in sorted(set(by_view_domain.get("features", {}).keys()) | set(by_view_domain.get("ui_components", {}).keys())):
                ...
                for ui in ui_components:
                    ...
                    if mapping_graph_id:
                        insert_link(conn, project_id, mapping_graph_id, ui["id"], interaction_node_id, "implements", True)
                    ...
                    for api in candidates[:2]:
                        insert_link(conn, project_id, journey_graph_id, interaction_node_id, api["id"], "depends_on", True)
                        insert_link(conn, project_id, default_graph_id, interaction_node_id, api["id"], "depends_on", True)
                        if mapping_graph_id:
                            insert_link(conn, project_id, mapping_graph_id, interaction_node_id, api["id"], "depends_on", True)
                        ...
                        for state_name in ["Idle", "Loading", "Error"]:
                            ...
                            conn.cursor().execute(
                                ...
                                (
                                    ...
                                    None,
                                    None,
                                    json.dumps({"generated_by": GEN_TAG, "feature_id": feat["id"], "state": state_name.lower()}),
                                    1,
                                ),
                            )
                            ensure_item_view(conn, state_id, project_id, journey_view_id)
