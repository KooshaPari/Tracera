...
class CoverageMetricsExtractor:
    ...
    def extract_from_pytest_output(self, log_file: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "tests_passed" in metrics or "tests_failed" in metrics:
                metrics["tests_total"] = (
                    metrics.get("tests_passed", 0) + metrics.get("tests_failed", 0)
                )
...
"""
        ...
        if "by_module" in metrics and metrics["by_module"]:
