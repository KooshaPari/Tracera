...
SKIP_DIRS = {
    '.git', '.venv', '.venv_test', 'node_modules', '.mypy_cache', '.ruff_cache',
    '.pytest_cache', '.uv-cache', '.playwright-mcp', '.sessions', '.trace',
...
ACRONYMS = {
    'api', 'id', 'url', 'ui', 'qa', 'db', 'sql', 'http', 'grpc', 'sso', 'jwt',
    'oauth', 'cdn', 'ui', 'ux', 'tls', 'ip', 'cpu', 'gpu'
}

SMALL_WORDS = {
    'a', 'an', 'the', 'and', 'or', 'to', 'of', 'in', 'at', 'on', 'for', 'by',
    'with', 'from', 'into', 'over', 'per', 'vs', 'via'
}
...
def normalize_token(token: str) -> str:
    token = CAMEL_RE_2.sub(r'\1 \2', token)
    token = CAMEL_RE_1.sub(r'\1 \2', token)
    return token
...
def humanize_title(title: str) -> str:
    ...
    for token in title.split():
        parts.append(normalize_token(token))
    title = ' '.join(parts)
...
def rewrite_sqlite(db_path: str):
    ...
    try:
        ...
        for item_id, title, item_metadata in cur.fetchall():
            original_title = ''
            if item_metadata:
                if isinstance(item_metadata, str) and 'original_title' in item_metadata:
