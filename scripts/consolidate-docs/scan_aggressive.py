...
class DocMetadata:
    ...
    consolidation_category: Optional[str] = None
...
def classify_consolidation_category(path: Path, content: str) -> Optional[str]:
    ...
    with Progress(
        ...
        for md_file in md_files:
            ...
            if md_file.name.lower() in ["readme.md", "readme"]:
                if not is_project_readme(md_file, root_dir):
                    progress.update(task, advance=1)
                    continue
