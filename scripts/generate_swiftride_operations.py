...
def generate_deployment_environments() -> List[Dict[str, Any]]:
    ...
    purposes = ["blue", "green", "canary", "sandbox", "testing", "qa", "uat", "perf"]
...
def generate_infrastructure_resources() -> List[Dict[str, Any]]:
    ...
    # S3 Buckets
    purposes = ["driver-documents", "rider-profiles", "ride-receipts", "app-logs",
                "analytics-data", "backup", "cdn-assets", "ml-models"]
    ...
    # SQS Queues
    queues = ["ride-requests", "notifications", "analytics-events", "driver-location-updates",
              "payment-processing", "fraud-detection", "dead-letter"]
    for queue in queues:
        items.append({
            "id": f"INFRA-{idx:03d}",
    ...
    functions = ["payment-webhook", "driver-onboarding", "fraud-detection",
                 "receipt-generator", "analytics-processor"]
