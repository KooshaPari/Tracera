...
def generate_title(view: str, item_type: str, index: int) -> str:
    ...
    if view == "code":
        ...
        return random.choice([
            f"services/auth{ext}", f"api/endpoints/user{ext}", f"models/data{ext}",
            f"utils/helpers{ext}", f"middleware/auth{ext}",
        ])
    if view == "tests":
        ...
        for _ in range(NUM_EVENTS):
            ...
            conn.execute(
                ...
                """),
                {
                    ...
                    "data": json.dumps({"action": random.choice(["created", "updated", "linked"]), "at": datetime.now(timezone.utc).isoformat()}),
                },
            )
        ...
        for i in range(1, 6):
            ...
            conn.execute(
                ...
                """),
                {
                    ...
                    "target_date": (datetime.now(timezone.utc) + timedelta(days=30 * i)).replace(tzinfo=None),
        ...
        for i in range(1, 6):
            conn.execute(
                ...
                """),
                {
                    ...
                    "start": (datetime.now(timezone.utc) - timedelta(days=14 * (6 - i))).replace(tzinfo=None),
                    "end": (datetime.now(timezone.utc) - timedelta(days=14 * (5 - i))).replace(tzinfo=None),
