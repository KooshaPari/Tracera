...
for title, desc, priority in ADDITIONAL_INITIATIVES:
    INITIATIVES.append({
        ...
        'description': desc,
        'priority': priority,
        'business_value': f'Strategic initiative to enhance SwiftRide market position'
    })


EPICS = [
    # Core Platform Epics (Initiative 1-3)
    {'title': 'Driver Onboarding Platform', 'initiative_idx': 0, 'description': 'Complete digital onboarding system for driver acquisition', 'duration_months': 6},
    {'title': 'Real-time Matching Engine', 'initiative_idx': 7, 'description': 'ML-powered rider-driver matching with <5s average match time', 'duration_months': 5},
    {'title': 'Payment Processing v2.0', 'initiative_idx': 7, 'description': 'Next-generation payment platform with fraud detection', 'duration_months': 4},
    {'title': 'Safety & Trust Platform', 'initiative_idx': 8, 'description': 'Comprehensive safety features including real-time monitoring', 'duration_months': 6},
    {'title': 'International Localization', 'initiative_idx': 0, 'description': 'Multi-language, multi-currency, multi-region support', 'duration_months': 5},
    {'title': 'EV Fleet Integration', 'initiative_idx': 1, 'description': 'Electric vehicle onboarding, charging infrastructure integration', 'duration_months': 4},
    {'title': 'Autonomous Vehicle SDK', 'initiative_idx': 2, 'description': 'Software development kit for AV integration', 'duration_months': 6},
    {'title': 'Enterprise Portal', 'initiative_idx': 3, 'description': 'B2B platform for corporate account management', 'duration_months': 5},
    {'title': 'Multi-Modal Integration', 'initiative_idx': 4, 'description': 'Integration with bikes, scooters, public transit', 'duration_months': 4},
    {'title': 'Premium Concierge', 'initiative_idx': 5, 'description': 'White-glove service for luxury tier', 'duration_months': 3},
]
...
async def create_item(session: AsyncSession, item_type: str, title: str, description: str,
                     priority: int = 5, status: str = 'draft', **kwargs) -> str:
...
async def generate_items():
    ...
    async with async_session() as session:
        ...
        for idx, epic in enumerate(EPICS[:60]):
            ...
            # Link to initiative
            initiative_id = items_db['initiative'][epic['initiative_idx'] % len(items_db['initiative'])]
        ...
        capabilities = [
            'Real-time GPS Tracking', 'Payment Processing', 'Identity Verification',
            'Route Optimization', 'Demand Forecasting', 'Fraud Detection',
            'Dynamic Pricing', 'Multi-language Support', 'Push Notifications',
            'In-App Messaging', 'Rating & Reviews', 'Surge Management',
            'Driver Matching', 'ETA Prediction', 'Location Services',
            'Trip Recording', 'Receipt Generation', 'Fare Calculation',
            'Promo Management', 'Referral Tracking', 'Analytics Pipeline',
            'Real-time Data Sync', 'Offline Mode', 'Background Location',
            'Emergency Services', 'Customer Support', 'Driver Dashboard',
            'Rider Dashboard', 'Admin Console', 'Reporting System',
            'Audit Logging', 'Compliance Monitoring', 'API Gateway',
            'Rate Limiting', 'Caching Layer', 'CDN Integration',
            'Image Processing', 'Video Streaming', 'Document Scanner',
            'OCR Processing', 'Voice Commands', 'Biometric Auth',
            'Two-Factor Auth', 'Session Management', 'Token Refresh',
            'Data Encryption', 'TLS/SSL', 'DDoS Protection',
        ...
        for epic_idx, epic_id in enumerate(items_db['epic'][:30]):  # First 30 epics
            ...
            for f_idx in range(num_features):
                ...
                feature_titles = [
                    f'Background check verification API integration',
                    f'Automated document upload and validation',
                    f'Real-time status dashboard',
                    f'Email and SMS notifications',
                    f'Mobile app workflow',
                    f'Admin approval interface',
                ...
                feature_id = await create_item(
                    ...
                    f'Feature implementation for enhanced system capability',
