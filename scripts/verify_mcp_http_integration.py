...
def verify_router_endpoints():
    ...
    mcp_routes = [route.path for route in app.routes if "/mcp" in str(route.path)]
...
def verify_transport_modes():
    ...
    # Test STDIO mode
    try:
        stdio_auth = build_auth_provider(transport="stdio")
...
def verify_fastapi_integration():
    ...
    mcp_routes = [route for route in app.routes if "/mcp" in str(route.path)]
