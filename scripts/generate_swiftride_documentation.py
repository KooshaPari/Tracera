...
## Earnings Example
- 20 rides/week × $15 average = $300/week
...
for i in range(78):
    ...
    RELEASE_NOTES.append({
        ...
        'type': release_type,
        'highlights': [
            f'Feature enhancement {i + 1}',
            f'Bug fix {i + 1}',
            f'Performance improvement {i + 1}'
        ],
        'details': f'''
## SwiftRide {version}
    ...
    async with async_session() as session:
        ...
        for idx, tutorial in enumerate(TUTORIALS):
            tutorial_id = await create_item(
                session, 'tutorial',
                tutorial['title'],
                f"Duration: {tutorial['duration_minutes']} min | Difficulty: {tutorial['difficulty']}\n\n{tutorial['content'][:500]}...",
                priority=8 if tutorial['difficulty'] == 'beginner' else 5
            )
        ...
        for idx, guide in enumerate(TROUBLESHOOTING_GUIDES):
            ts_id = await create_item(
                session, 'troubleshooting_guide',
                guide['title'],
                f"Category: {guide['category']} | Severity: {guide['severity']}\n\n{guide['solution'][:500]}...",
                priority=10 if guide['severity'] == 'critical' else 8
            )
