...
def seed_items(session: Session, project_id: str, views: dict[str, View], kinds: dict[str, NodeKind]) -> dict[str, Item]:
    items: dict[str, Item] = {}
    ...
    add_item("UI_C_TASK_DELETE_BUTTON", "Task Delete Button", "journey", "component", metadata={"type": "button", "label": "Delete"})
    add_item("UI_I_TOGGLE_TASK", "Toggle Task Status", "journey", "interaction", metadata={"trigger": "onChange"})
    add_item("UI_I_INLINE_EDIT_TITLE", "Inline Edit Task Title", "journey", "interaction", metadata={"trigger": "onBlur"})
    ...
    add_item("T_FIELD_TASK_ID", "tasks.id", "technical_requirements", "data_entity", metadata={"field": "id", "type": "uuid"})
    add_item("T_FIELD_TASK_TITLE", "tasks.title", "technical_requirements", "data_entity", metadata={"field": "title", "type": "text"})
    add_item("T_FIELD_TASK_STATUS", "tasks.status", "technical_requirements", "data_entity", metadata={"field": "status", "type": "text"})
    add_item("T_FIELD_TASK_CREATED", "tasks.created_at", "technical_requirements", "data_entity", metadata={"field": "created_at", "type": "timestamptz"})
...
def seed_external_links(session: Session, project_id: str) -> None:
    existing = session.execute(
        select(ExternalLink).where(ExternalLink.project_id == project_id)
    ).scalar_one_or_none()
    if existing:
        return
    session.add(
...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        views = ensure_views(session, project.id)
        ...
        graphs = ensure_graphs(session, project.id)
        items = seed_items(session, project.id, views, kinds)
        seed_graph_nodes(session, project.id, graphs, items)
        seed_links(session, project.id, graphs)
        seed_external_links(session, project.id)
        session.commit()
