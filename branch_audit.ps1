$branches = @(
    'QUARANTINE-destructive-consolidate-20260613',
    'ax-dx-scaffold',
...
foreach ($b in $branches) {
    $ahead = (git log --oneline main..$b 2>$null | Measure-Object -Line).Lines
    $dels = (git diff --name-status main..$b 2>$null | Select-String '^D' | Measure-Object -Line).Lines
    $results += "$b|$ahead|$dels"
}
$results | Out-File -FilePath E:\Dev\Tracera\branch_audit_results.txt -Encoding utf8
Write-Output "DONE"
